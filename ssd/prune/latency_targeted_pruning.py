    ...
    for group in groups:
        ...
        for layer_name in group:
            ...
            if layer_masks is not None and pre_group_name in groups:
                ...
                pre_active_neuron_num = layers[pre_group_name[0]].weight_orig.size(0) if pre_group_name is not None else 3
    ...
    for group in groups:
        ...
        for layer_name in group:
            if layer_masks is not None and layer_name in layer_masks:
                ...
                active_neuron_num = int(layers[layer_name].weight_orig.size(0))
...
def apply_latency_target_pruning(output_dir, layers, layer_bn, blocks, layer_gate, metric, target_latency, layer_masks,
    ...
    if step_size > 0:
        for k in channel_group_size_dict.keys():
            channel_group_size_dict[k] = min(step_size, int(layers[k].weight_orig.size(0)))
...
#             print(cur_groups)
    ...
    #     print(model.decision_vars[group_var_slices[group_name][0]:group_var_slices[group_name][1]])
    # print(items_in_bag)
    return items_in_bag, None, Nonee