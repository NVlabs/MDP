...
class NetOptimizer():
    def __init__(self, model, len_train_dataset, train_loader, args, layer_structure=None, gpu=None):
        ...
        if self.method >= 20:
            ...
            self.prune_target = set_latency_prune_target(initial_latency, self.gs_start, self.gs_end, args.target_latency, args.prune_ratio, mode=args.prune_method)
    ...
    def optimize_net(self):
        ...
        for epoch_id in tqdm(range(0, self.epochs)):            
            ...
            if self.prune_target is not None and self.gs_start <= epoch_id < self.gs_end and self.prune_target[epoch_id-self.gs_start] > 0:
                if self.method in [15, 16]:
                    hybrid_norm_importance(self.layers, self.groups, self.neuron_metric_value, self.method)
                if self.method < 20:
                    total_pruned_num = apply_pruning(self.layers, self.layer_bn, self.layer_gate, self.neuron_metric_value,
                                                    self.prune_target[epoch_id-self.gs_start],
                                                    self.pre_pruned_num, self.pre_target_num,
                                                    self.method, self.group_mask)
                    pre_pruned_num += total_pruned_num
                    pre_target_num += self.prune_target[epoch_id-self.gs_start]
                    if self.is_main:
                        print('Total actually pruned number {} vs. Scheduled number {}'.format(pre_pruned_num, pre_target_num))
                else:
                    total_pruned_num = apply_latency_target_pruning(self.layers, self.layer_bn, self.layer_gate, self.neuron_metric_value, 
                                                                    self.prune_target[epoch_id-self.gs_start], self.group_mask,
