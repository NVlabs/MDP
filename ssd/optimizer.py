...
def set_masks(layers, layer_masks, layer_bn=None):
...
class NetOptimizer():
    def __init__(self, model, len_train_dataset, train_loader, args, layer_structure=None, gpu=None):
        ...
        if self.method >= 20:
            ...
            # layer_masks_path = '/workspace/marvin-latency/ssd512_resnet50_prune0.449_backbone_0-200/2021-05-02_19-37-04/layer_masks.pkl'
            # with open(layer_masks_path, 'rb') as f:
            #     self.layer_masks = pkl.load(f)
            # prune_latency, backbone_latency, head_latency = get_total_latency(self.layers, self.groups, self.pre_group, self.layer_masks, self.fmap_table, self.lookup_table)
            # print(prune_latency, "prune latency of model!")
            # self.layer_masks = {}
            
            initial_latency, backbone_latency, head_latency = get_total_latency(self.layers, self.groups, self.pre_group, self.layer_masks, self.fmap_table, self.lookup_table)
    ...
    def optimize_net(self):
        ...
        for epoch_id in tqdm(range(0, self.epochs)):            
            ...
            # if self.prune_target is not None and len(self.prune_target) > 0 and self.gs_start <= epoch_id and self.prune_target[0] > 0 and not self.to_grow:
                ...
                # exit()
                blocks_num = None
                total_latency = apply_latency_target_pruning(self.output_dir, self.layers, self.layer_bn, blocks, self.layer_gate, self.neuron_metric_value, 
                                                            target_latency, self.layer_masks,
                                                            self.groups, self.pre_group, self.aft_group_list, self.fmap_table, self.lookup_table, self.method,
                                                            lut_bs=64, pulp=False, blocks_num=blocks_num, no_blockprune=False)
                
                set_masks(self.layers, self.layer_masks, self.layer_bn)
                # set_masks(layers, layer_masks, layer_bn, mask_bias=True)
                ...
                if self.layer_masks is not None:
                    with open('{}/layer_masks.pkl'.format(self.output_dir), 'wb') as outfile:
                        pkl.dump(self.layer_masks, outfile)
    ...
    def train_epoch(self, epoch_id):
        ...
        for batch_id, (images, boxes, labels, _) in enumerate(self.train_loader):
            ...
            # if self.layer_masks is not None:
            #     for group, mask in self.layer_masks.items():
            #         for layer_name in group:
            ...
            if self.update_metric:
                update_neuron_metric_hahp(self.layers, self.groups, layer_bn=self.layer_bn, layer_gate=self.layer_gate, method=self.method, neuron_metric=self.neuron_metric_value)
