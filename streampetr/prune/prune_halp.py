...
from prune.latency_targeted_pruning import apply_latency_target_pruning, get_total_latency, set_latency_prune_target
...
class PruneHookHalp(Hook):
    def __init__(self, model, result_dir):
        ...
        for group in groups:
            self.group_mask[group] = torch.ones(self.layers[group[0]].weight.size(0)).to(self.layers[group[0]].weight.device)
    ...
    def prepare_pruning(self):
        initial_latency = get_total_latency(self.layers, self.groups, self.pre_group, self.group_mask, self.fmap_table, self.lookup_table, self.lut_bs)
    ...
    def after_train_iter(self, runner):
        ...
        if cur_iter > self.imp_start:
            ...
            update_neuron_metric(self.layers, self.groups, self.layer_bn, neuron_metric=self.neuron_metric)
        ...
        if self.prune_counter == self.prune_interval and self.prune_target:
            ...
            for layer_name in self.neuron_metric.keys():
                self.neuron_metric[layer_name] = torch.div(self.neuron_metric[layer_name], self.prune_interval)
            
            blocks = build_block_from_layers(self.layers)
            output_dir = "./"
            layer_gate = None
            target_latency = self.prune_target.pop(0)
            apply_latency_target_pruning(output_dir, self.layers, self.layer_bn, blocks, layer_gate, self.neuron_metric, target_latency, self.group_mask,\
                    self.groups, self.pre_group, self.aft_group_list, self.fmap_table, self.lookup_table, self.method, \
                    wrap_solver=None, mu=0., step_size=-1, lut_bs=self.lut_bs, pulp=False)
            self.neuron_metric = {}
            with open(f"{self.result_dir}/group_mask.pkl", 'wb') as f:
                pkl.dump(self.group_mask, f)
            self.prune_counter = 0
            # exit()
        self.mask_weights()
