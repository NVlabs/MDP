...
def get_args_parser():
    ...
    parser.add_argument("--name_ext",
                        type=str,
                        default="",
                        required=False,
                        help="extension name of the experiment")
...
def main(args):
    ...
    if args.name_ext:
        args.output_dir += '_' + args.name_ext
    if args.pruning:
    ...
    loss_scaler = None
    if use_amp == 'apex':
        teacher_model, optimizer_teacher = amp.initialize(teacher_model, optimizer_teacher, opt_level=OPT_LEVEL)
...
#        model_without_ddp = model.module
    ...
    pruning_engine = initialize_pruning_engine(named_modules=named_modules, pruning_config=pruning_config,
                                                   exp_name=exp_name, prune_by="predefined", tensorboard=True, latency_regularization=args.latency_regularization, latency_target=args.latency_target, latency_look_up_table=args.latency_look_up_table, pruning_steps=args.pruning_steps)
