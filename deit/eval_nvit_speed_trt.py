...
def main(args):
    ...
    # base_macs, base_params = tp.utils.count_ops_and_params(dense_model_without_ddp, example_inputs)
    # print("MACs: %.4f G, Params: %.4f M"%(base_macs/1e9, base_params/1e6))
    # Measure Latency
    # example_inputs = torch.randn(256, *input_size).to(device)
    example_inputs = torch.randn(BS, *input_size).to(device)
    # trt_model = torch_tensorrt.compile(dense_model_without_ddp, ir="dynamo", inputs=[example_inputs])
    trt_model = torch.compile(dense_model_without_ddp, backend="tensorrt")
    trt_model(example_inputs)
    print("TRT conversion successful")
    # runtime = estimate_latency(dense_model_without_ddp, example_inputs)
    runtime = estimate_latency(trt_model, example_inputs)
    runtime = runtime / BS
    FPS = (1 / runtime) * 1000
    print(f"FPS: {FPS}, Runtime in ms: {runtime}")
    return
