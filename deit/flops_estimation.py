...
def main():
    ...
    save_name = f"FLOPs_{lut_name}_lut_BS{BS}_NUM_TOKENS{NUM_TOKENS}_v100.pkl"
        ...
        for head_idx in head_idx_range:
            ...
            for emb_idx in emb_idx_range:
                for qk_idx in qk_idx_range:
                    ...
                    QK_model.eval()
                    base_macs, base_params = tp.utils.count_ops_and_params(QK_model, dummy_QK_input)
                    base_macs = base_macs / 1e6
                    lut[head_idx, emb_idx, qk_idx] = base_macs
        ...
        for head_idx in head_idx_range:
            ...
            for emb_idx in emb_idx_range:
                for v_idx in v_idx_range:
                    ...
                    dummy_inputs = {'x': dummy_V_input, 'attn': dummy_attn}
                    base_macs, base_params = tp.utils.count_ops_and_params(V_AND_PROJ_MODEL, dummy_inputs)
                    base_macs = base_macs / 1e6
                    base_macs = base_macs / 256
                    lut[head_idx, emb_idx, v_idx] = base_macs
    ...
    # emb, mlp
        ...
        for emb_idx in emb_idx_range:
            ...
            for mlp_idx in mlp_idx_range:
                ...
                MLP_MODEL.eval()
                base_macs, base_params = tp.utils.count_ops_and_params(MLP_MODEL, dummy_mlp_input)
                base_macs = base_macs / 1e6
                lut[emb_idx, mlp_idx] = base_macs
