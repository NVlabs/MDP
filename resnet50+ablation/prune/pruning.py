...
def update_neuron_metric_mdp(layers, groups, layer_bn=None, layer_gate=None, method=0, neuron_metric={},
...
def calc_importance_fused_convkbn(layers, group, layer_bn=None, layer_gate=None, method=2, fmap_table=None):
    
    bn_layer = layers[layer_bn[group[0]]]
    layer = layers[group[0]]
    w_bn = torch.diag(bn_layer.weight.div(torch.sqrt(bn_layer.eps+bn_layer.running_var))).cuda()
    w_conv = layer.weight.data
    if w_conv.shape[-1] == 3:
        fm_size = fmap_table[group[0]]
        w_conv = reweight_basedon_reorg(w_conv, fm_size)
        # exit()
