...
from prune.latency_targeted_pruning_mdlm import apply_latency_target_pruning
...
def train_loop(args, model, criterion, optimizer, lr_scheduler, train_loader, train_loader_len, val_loader, epochs, fp16,
    ...
    if method == 9 or method == 10 or method >= 20:  # latency aware pruning / latency targeted pruning
        ...
        from prune.latency_targeted_pruning_mdlm import load_group_size
    ...
    if method >= 20:
        from prune.latency_targeted_pruning_mdlm import set_latency_prune_target, get_total_latency
...
def train(train_loader, train_loader_len, model, criterion, optimizer, lr_scheduler, fp16, epoch_id,
          ...
          wrap_solver=None, clipping=None, lut_bs=256, pruned=False, pulp=False, pyomo=False):
    """ Train one epoch """
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        # if epoch_id == prune_epoch and prune_target is not None and i == 100 and i<=1200:
