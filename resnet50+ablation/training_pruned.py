...
from prune.pruning import apply_pruning, update_neuron_metric, hybrid_norm_importance
from prune.latency_targeted_pruning import apply_latency_target_pruning
...
def train_loop(args, model, criterion, optimizer, lr_scheduler, train_loader, train_loader_len, val_loader, epochs, fp16,
    ...
    if pre_mask is not None:
        ...
        for group_name in group_mask.keys():
            group_mask[group_name] = group_mask[group_name].cpu().cuda()
    ...
    if method >= 20:
        ...
        initial_latency = get_total_latency(layers, groups, pre_group, group_mask, fmap_table, lookup_table, args.lut_bs)
        if args.prune_per_epoch:
            prune_target = set_latency_prune_target(initial_latency, prune_start, prune_end, args.target_latency, args.prune_ratio, mode=args.prune_mode)
        else:
            prune_target = set_latency_prune_target(initial_latency, 0, 30, args.target_latency, args.prune_ratio, mode=args.prune_mode)
        print(prune_target)
    ...
    for epoch_id in range(epochs):
        ...
        loss, neuron_metric_value = train(train_loader, train_loader_len,
                                          model, criterion,
                                          optimizer,
                                          lr_scheduler,
                                          fp16,
        ...
        if prune_per_epoch:
            if prune_target is not None and prune_start <= epoch_id < prune_end and prune_target[epoch_id-prune_start] > 0:
                if method in [15, 16]:
                    hybrid_norm_importance(layers, groups, neuron_metric_value, method)
                if method < 20:
                    total_pruned_num = apply_pruning(layers, layer_bn, layer_gate, neuron_metric_value,
                                                    prune_target[epoch_id-prune_start],
                                                    pre_pruned_num, pre_target_num,
                                                    method, group_mask)
                    pre_pruned_num += total_pruned_num
                    pre_target_num += prune_target[epoch_id-prune_start]
                    print('Total actually pruned number {} vs. Scheduled number {}'.format(pre_pruned_num, pre_target_num))
                else:
                    total_pruned_num = apply_latency_target_pruning(layers, layer_bn, layer_gate, neuron_metric_value, 
                                                                    prune_target[epoch_id-prune_start], group_mask,
                                                                    groups, pre_group, aft_group_list, fmap_table, lookup_table, method,
                                                                    wrap_solver=wrap_solver, mu=args.mu, step_size=args.step_size,
                                                                    lut_bs=args.lut_bs)

                cur_conv_neurons = count_non_zero_neurons(layers)
                print(ori_conv_neurons, total_pruned_num, cur_conv_neurons)
                #assert ori_conv_neurons - total_pruned_num == cur_conv_neurons
                prec1, prec5 = validate(val_loader, model, criterion)
                if verbose:
                    train_writer.add_scalars('accuracy', {'val/after_prune': prec1}, epoch_id)
                    train_writer.add_scalar('neuron_num', cur_conv_neurons, epoch_id)
        ...
        if verbose:
            if group_mask is not None:
                with open('{}/group_mask.pkl'.format(output_dir), 'wb') as outfile:
                    pkl.dump({group_name: mask.cpu() for group_name, mask in group_mask.items()}, outfile)
    return best_prec1
...
def get_train_step(model, criterion, optimizer, fp16, use_amp=False,
                   ...
                   neuron_metric=None, group_mask=None, get_hessian=False, bn_weights=None,
                   pre_group=None, aft_group_list=None, step_size=8, mu=6e-4, fmap_table=None, lookup_table=None, clipping=None):
    def _step(input, target, optimizer_step=True, update_metric=False, last_iter=False):
        optimizer.zero_grad()
        ...
        if update_metric:
            update_neuron_metric(layers, groups, layer_bn, layer_gate, method, neuron_metric,
                                 pre_group, aft_group_list, step_size, mu, group_mask,
                                 fmap_table, lookup_table, last_iter)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, lr_scheduler, fp16, epoch_id,
          use_amp=False, batch_size_multiplier=1, train_log_freq=100,
          layers=None, groups=None, layer_bn=None, layer_gate=None, method=0, group_mask=None,
          ...
          wrap_solver=None, clipping=None, lut_bs=256):
    """ Train one epoch """
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss, prec1, prec5 = step(input, target, optimizer_step=optimizer_step, update_metric=update_metric, last_iter=(i==train_loader_len-1))
        ...
        if epoch_id == prune_epoch and prune_target is not None and (i+1)%40==0 and i<=1200:
            if method in [0, 1, 6]:
                ...
                for group_name in neuron_metric_value.keys():
                    neuron_metric_value[group_name] = torch.div(neuron_metric_value[group_name], 40)
            if method in [15, 16]:
                hybrid_norm_importance(layers, groups, neuron_metric_value, method)
            if method < 20:
                total_pruned_num = apply_pruning(layers, layer_bn, layer_gate, neuron_metric_value,
                                                ...
                                                method, group_mask)
                pre_pruned_num += total_pruned_num
                pre_target_num += prune_target[i//40]
                print('Total actually pruned number {} vs. Scheduled number {}'.format(pre_pruned_num, pre_target_num))
            else:
                total_pruned_num = apply_latency_target_pruning(layers, layer_bn, layer_gate, neuron_metric_value, 
                                                                prune_target[i//40], group_mask,
                                                                groups, pre_group, aft_group_list, fmap_table, lookup_table, method,
