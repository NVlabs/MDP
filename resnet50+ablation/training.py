...
def get_train_step(model, criterion, optimizer, fp16, use_amp=False,
    ...
    def _step(input, target, optimizer_step=True, update_metric=False, last_iter=False, imp_convk=False, imp_fused_convk=False):
        optimizer.zero_grad()
        ...
        if group_mask is not None:
            for group, mask in group_mask.items():
                for layer_name in group:
                    ...
                    # if sum(mask == 0) > 0:
                    #     print(layer_name, weight.grad.data[mask==0.].sum().item())
        ...
        if update_metric:
            update_neuron_metric(layers, groups, layer_bn, layer_gate, method, neuron_metric,
                                 ...
                                 fmap_table, lookup_table, last_iter, imp_convk=imp_convk, imp_fused_convk=imp_fused_convk)
...
def train(train_loader, train_loader_len, model, criterion, optimizer, lr_scheduler, fp16, epoch_id,
          ...
          wrap_solver=None, clipping=None, lut_bs=256, pruned=False, pulp=False, pyomo=False, args=None):
    """ Train one epoch """
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        loss, prec1, prec5 = step(input, target, optimizer_step=optimizer_step, update_metric=update_metric, last_iter=(i==train_loader_len-1), imp_convk=args.imp_convk, imp_fused_convk=args.imp_fused_convk)
